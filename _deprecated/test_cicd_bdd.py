"""
pytest-bdd entry point: CI/CD pipeline token detection scenarios.
"""
...
scenarios("../features/cicd.feature")


# ---------------------------------------------------------------------------
# Parametrised regression — all 8 CI/CD entity types
# ---------------------------------------------------------------------------
from fixtures.entity_samples import CICD_SAMPLES
from conftest import BASE_URL, HTTP_TIMEOUT

_SANITIZE_URL = f"{BASE_URL}/sanitize"


@allure.feature("CI/CD Tokens")
@allure.story("Regression — all CI/CD entities")
@pytest.mark.cicd
@pytest.mark.regression
@pytest.mark.parametrize("entity_type,sample", CICD_SAMPLES.items())
def test_cicd_entity_sanitized(entity_type, sample):
    """Each CI/CD entity type is detected and masked."""
    payload = {
        "text":     sample["text"],
        "language": "en",
    ...
    allure.dynamic.title(f"CI/CD: {entity_type}")
